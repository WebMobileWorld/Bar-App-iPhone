...
#import "BeerDirectoryCell.h"
#import "BeerDirectoryDetailViewController.h"
#import "BeerSuggestViewController.h"
@interface BeerDirectoryViewController ()<MBProgressHUDDelegate,DirectoryHeaderViewDelegate,AlphabetSearchViewDelegate,TTTAttributedLabelDelegate,FPPopoverControllerDelegate,UIActionSheetDelegate>
{
    //HUD
    MBProgressHUD *HUD;
...
@implementation BeerDirectoryViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.titleView = [CommonUtils setTitleLabel:@"BEER DIRECTORY"];;
...
- (IBAction)btnSuggestClicked:(id)sender {
    ...
    BeerSuggestViewController *suggest = [[BeerSuggestViewController alloc] initWithNibName:@"BeerSuggestViewController" bundle:nil];
...
-(void)setDataStructureForLGView {
    _titlesArray = @[@{@"title":@"Sort By",@"value":@""},
                     @{@"title":@"BEER NAME A-Z",@"value":@"beer_name#ASC"},
                     @{@"title":@"BEER NAME Z-A",@"value":@"beer_name#DESC"},
                     @{@"title":@"BREWED BY A-Z",@"value":@"producer#ASC"},
                     @{@"title":@"BREWED BY Z-A",@"value":@"producer#DESC"},
                     @{@"title":@"CITY PRODUCED A-Z",@"value":@"city_produced#ASC"},
                     @{@"title":@"CITY PRODUCED Z-A",@"value":@"city_produced#DESC"}
                     ];
    selectedIndex = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        case 0:
        {
            static NSString *simpleTableIdentifier = @"BeerDirectoryCell";
            
            BeerDirectoryCell *cell = (BeerDirectoryCell *)[tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
            if (cell == nil)
            {
                NSArray *nib;
                if([CommonUtils isiPad])
                {
                    nib = [[NSBundle mainBundle] loadNibNamed:@"BeerDirectoryCell" owner:self options:nil];
                }
                else
                {
                    nib = [[NSBundle mainBundle] loadNibNamed:@"BeerDirectoryCell" owner:self options:nil];
            ...
            BeerDirectory *beer = [self.aryList objectAtIndex:indexPath.row];
            ...
            cell.lblCityProduced.text = [CommonUtils removeHTMLFromString:beer.beer_city_produced];
            cell.lblType.text = [CommonUtils removeHTMLFromString:beer.beer_type];
            cell.lblWebSite.text = [CommonUtils removeHTMLFromString:beer.beer_website];
            ...
            NSString *beer_logo = [beer beer_image];
            NSString *strImgURL = [NSString stringWithFormat:@"%@%@%@",WEBIMAGE_URL,BEERLOGO,beer_logo];
            NSURL *imgURL = [NSURL URLWithString:strImgURL];
            
            [cell.imgBeerLogo sd_setImageWithURL:imgURL placeholderImage:[UIImage imageNamed:@"no_beer.png"]];
...
-(void)beer_lists
{
    ...
    NSString *myURL = [NSString stringWithFormat:@"%@beer_lists",WEBSERVICE_URL];
    NSMutableURLRequest *Request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString: myURL]];
    NSString *params = nil;
    NSString *bar_id = [self getBarIdForWebservice];    
    
    params = [NSString stringWithFormat:@"user_id=%@&device_id=%@&unique_code=%@&limit=%ld&offset=%ld&keyword=%@&alpha=%@&order_by=%@&bar_id=%@",user_id,device_id,unique_code,(long)limit,(long)offset,keyWord,alpha,order_by,bar_id];
    ...
    [NSURLConnection sendAsynchronousRequest:Request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (connectionError == nil) {
            ...
            if ([[tempDict valueForKey:@"status"] isEqualToString:@"success"]) {
                totalRecords = [[CommonUtils getNotNullString:[tempDict valueForKey:@"beerlist_total"]] integerValue];
                offset = offset + limit;
                NSArray *aryBeerList = [[tempDict valueForKey:@"beerlist"] valueForKey:@"result"];
                self.aryList = [self getBeersFromResult:aryBeerList];
...
-(NSMutableArray *)getBeersFromResult:(NSArray *)aryBeerList
{
    ...
    for (NSDictionary *dict in aryBeerList) {
        BeerDirectory *beer = [BeerDirectory getBeerWithDictionary:dict];
        [aryBeers addObject:beer];
    }
    return aryBeers;
}
    ...
    [NSURLConnection sendAsynchronousRequest:Request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (connectionError == nil) {
            ...
            if ([[tempDict valueForKey:@"status"] isEqualToString:@"success"]) {
                ...
                NSArray *aryBeerList = [[tempDict valueForKey:@"beerlist"] valueForKey:@"result"];
                [self getMoreBeersFromResult:aryBeerList];
            }
        }
        else {
...
-(void)getMoreBeersFromResult:(NSArray *)aryBeerList {
    
    for(int i=0; i<[aryBeerList count]; i++)
    {
        NSDictionary *dict = [aryBeerList objectAtIndex:i];
        BeerDirectory *beer = [BeerDirectory getBeerWithDictionary:dict];
        [self.aryList addObject:beer];
    }
    offset = offset + limit;
    [self.tblView reloadData];
}
