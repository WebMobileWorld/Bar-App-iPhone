//
//  CocktailDirectoryViewController.m
//  AmericanDiveBars
//
...
#import "CocktailDirectoryCell.h"
#import "CocktailDirectoryDetailViewController.h"

#import "CocktailSuggestViewController.h"

@interface CocktailDirectoryViewController ()<MBProgressHUDDelegate,DirectoryHeaderViewDelegate,AlphabetSearchViewDelegate,TTTAttributedLabelDelegate,FPPopoverControllerDelegate,UIActionSheetDelegate>
{
    //HUD
    MBProgressHUD *HUD;
...
@implementation CocktailDirectoryViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.titleView = [CommonUtils setTitleLabel:@"COCKTAIL RECIPES"];
    [self configureTopButtons];
    [self initializeDataStructure];
    [self configureHeaderView];
    [self configureAlphabetView];
    [self callCocktailListWebservice];
...
- (IBAction)btnSuggestClicked:(id)sender {
    ...
    CocktailSuggestViewController *suggest = [[CocktailSuggestViewController alloc] initWithNibName:@"CocktailSuggestViewController" bundle:nil];
...
-(void)btnFilterClicked:(id)sender
{
    ...
    controller.cocktailDelegate = self;
...
-(void)setDataStructureForLGView {
    _titlesArray = @[@{@"title":@"Sort By",@"value":@""},
                     @{@"title":@"COCKTAIL NAME A-Z",@"value":@"cocktail_name#ASC"},
                     @{@"title":@"COCKTAIL NAME Z-A",@"value":@"cocktail_name#DESC"}
                     ];
    selectedIndex = 0;
...
-(void)configureHeaderView  {
    ...
    [directoryHeaderView configureBeerHeaderView:@"Search Cocktail by Name"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        case 0:
        {
            static NSString *simpleTableIdentifier = @"CocktailDirectoryCell";
            
            CocktailDirectoryCell *cell = (CocktailDirectoryCell *)[tableView dequeueReusableCellWithIdentifier:simpleTableIdentifier];
            if (cell == nil)
            {
                NSArray *nib;
                if([CommonUtils isiPad])
                {
                    nib = [[NSBundle mainBundle] loadNibNamed:@"CocktailDirectoryCell" owner:self options:nil];
                }
                else
                {
                    nib = [[NSBundle mainBundle] loadNibNamed:@"CocktailDirectoryCell" owner:self options:nil];
            ...
            CocktailDirectory *cocktail = [self.aryList objectAtIndex:indexPath.row];
            
            cell.lblCocktailTitle.text = [CommonUtils removeHTMLFromString:cocktail.cocktail_name];
            cell.lblDesc.text = [CommonUtils removeHTMLFromString:cocktail.cocktail_ingredients];
            cell.lblType.text = [CommonUtils removeHTMLFromString:cocktail.cocktail_type];
            cell.lblServed.text = [CommonUtils removeHTMLFromString:cocktail.cocktail_served];
            cell.lblDifficulty.text = [CommonUtils removeHTMLFromString:cocktail.cocktail_difficulty];
            
            NSString *cocktail_logo = [cocktail cocktail_image];
            ...
            [cell.imgCocktailLogo sd_setImageWithURL:imgURL placeholderImage:[UIImage imageNamed:@"no_cocktail.png"]];
        ...
        case 1:
        {
            ...
            if(offset < totalRecords && totalRecords>10)
            {
                ...
                if (hasConnected)
                {
                    [self callCocktailListWebservice_LOAD_MORE];
                }
                else
                {
...
-(void)callCocktailListWebservice {
    [CommonUtils callWebservice:@selector(cocktail_lists) forTarget:self];
}

-(void)callCocktailListWebservice_LOAD_MORE {
    [CommonUtils callWebservice:@selector(cocktail_lists_LOAD_MORE) forTarget:self];
}
...
-(void)cocktail_lists
{
    ...
    //192.168.1.27/ADB/api/cocktail_lists
    NSString *myURL = [NSString stringWithFormat:@"%@cocktail_lists",WEBSERVICE_URL];
    ...
    [NSURLConnection sendAsynchronousRequest:Request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (connectionError == nil) {
            ...
            if ([[tempDict valueForKey:@"status"] isEqualToString:@"success"]) {
                totalRecords = [[CommonUtils getNotNullString:[tempDict valueForKey:@"cocktaillist_total"]] integerValue];
                offset = offset + limit;
                NSArray *aryCocktailList = [[tempDict valueForKey:@"cocktaillist"] valueForKey:@"result"];
                self.aryList = [self getCocktailsFromResult:aryCocktailList];
...
-(NSMutableArray *)getCocktailsFromResult:(NSArray *)aryCocktailList
{
    ...
    for (NSDictionary *dict in aryCocktailList) {
        CocktailDirectory *cocktail = [CocktailDirectory getCocktailWithDictionary:dict];
        [aryCocktails addObject:cocktail];
    }
    return aryCocktails;
}

#pragma mark bar_lists WITH LOAD MORE
-(void)cocktail_lists_LOAD_MORE
{
    User *user = [CommonUtils getUserLoginDetails];
    NSString *user_id = [user user_id];
    NSString *device_id = [user device_id];
    ...
    [NSURLConnection sendAsynchronousRequest:Request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (connectionError == nil) {
            ...
            if ([[tempDict valueForKey:@"status"] isEqualToString:@"success"]) {
                ...
                NSArray *aryCocktailList = [[tempDict valueForKey:@"cocktaillist"] valueForKey:@"result"];
                [self getMoreCocktailsFromResult:aryCocktailList];
            }
        }
        else {
...
-(void)getMoreCocktailsFromResult:(NSArray *)aryCocktailList {
    
    for(int i=0; i<[aryCocktailList count]; i++)
    {
        NSDictionary *dict = [aryCocktailList objectAtIndex:i];
        CocktailDirectory *cocktail = [CocktailDirectory getCocktailWithDictionary:dict];
        [self.aryList addObject:cocktail];
    }
    offset = offset + limit;
    [self.tblView reloadData];
}
