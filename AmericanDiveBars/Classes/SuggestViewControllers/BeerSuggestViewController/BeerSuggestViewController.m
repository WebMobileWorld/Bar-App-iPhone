...
@interface BeerSuggestViewController ()<MBProgressHUDDelegate,UzysAssetsPickerControllerDelegate,FPPopoverControllerDelegate>
{
    MBProgressHUD *HUD;
...
@implementation BeerSuggestViewController
...
-(void)btnFilterClicked:(id)sender
{
    ...
    controller.suggestBeerDelegate = self;
...
{
    return [self BeerSuggestCellAtIndexPath:indexPath];
}

#pragma mark
#pragma mark BeerSuggestCell Cell Configuration
- (BeerSuggestCell *)BeerSuggestCellAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString * const BeerSuggestCellIdentifier = @"BeerSuggestCell";
    
    BeerSuggestCell *cell = (BeerSuggestCell *)[self.tblView dequeueReusableCellWithIdentifier:BeerSuggestCellIdentifier];
    if (cell == nil)
    {
        NSArray *nib;
        if([CommonUtils isiPad])
        {
            nib = [[NSBundle mainBundle] loadNibNamed:@"BeerSuggestCell-ipad" owner:self options:nil];
        }
        else
        {
            nib = [[NSBundle mainBundle] loadNibNamed:@"BeerSuggestCell" owner:self options:nil];
        }
        cell = [nib objectAtIndex:0];
    }
    
    cell.txtBeerTitle.text = beerName;
    cell.txtDesc.text = desc;
    cell.txtType.text = type;
    cell.txtABV.text = ABV;
    
    cell.txtBrewedBy.text = brewedBy;
    cell.txtCityProduced.text = cityProduced;
    cell.txtState.text = state;
    cell.txtCountry.text = country;
    cell.txtWeb.text = website;
    ...
    if (didSelectImage) {
        ...
        cell.imgUser.image = [UIImage imageNamed:@"no_beer_detail.png"];
    }
...
-(void)btnSave_Clicked:(UIButton *)btnSave {
    ...
    BeerSuggestCell *cell = (BeerSuggestCell *)[self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:1]];
    
    if ([beerName length]==0) {
        ShowAlert(AlertTitle, @"Please provide the beer name.");
        cell.txtBeerTitle.layer.borderColor = [UIColor redColor].CGColor;
        cell.txtBeerTitle.layer.borderWidth = 1.5f;
        
    }
    else if ([country length] == 0) {
        cell.txtCountry.layer.borderColor = [UIColor redColor].CGColor;
        cell.txtCountry.layer.borderWidth = 0.0f;

        ShowAlert(AlertTitle, @"Please provide the country name.");
    }
    else {
        cell.txtBeerTitle.layer.borderColor = [UIColor clearColor].CGColor;
        cell.txtBeerTitle.layer.borderWidth = 0.0f;

        cell.txtCountry.layer.borderColor = [UIColor clearColor].CGColor;
        cell.txtCountry.layer.borderWidth = 0.0f;

        
        [self callBeerSuggestWebservice];
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if (textField.tag == 104) {
        brewedBy = textField.text;
    }
    
    else if (textField.tag == 105) {
        cityProduced = textField.text;
    }
...
-(void)configureTextView:(BeerSuggestCell *)cell withEditable:(BOOL)isEditable
{
...
-(void)callBeerSuggestWebservice {
    [CommonUtils callWebservice:@selector(beerSuggest) forTarget:self];
}
...
-(void)beerSuggest {
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@beersuggest",WEBSERVICE_URL];
    ...
    //beer_name
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"beer_name"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",beerName] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //beer_state
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"beer_state"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",state] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //beer_country
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"beer_country"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",country] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //beer_desc
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"beer_desc"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",desc] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //beer_type
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"beer_type"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",type] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //abv
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"abv"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",ABV] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //producer
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"producer"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",brewedBy] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //city_produced
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"city_produced"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",cityProduced] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //beer_website
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"beer_website"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",website] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (connectionError == nil) {
            ...
            if ([status isEqualToString:@"success"]) {
                ...
                [self.navigationController popViewControllerAnimated:YES];
                ShowAlert(AlertTitle, @"Your beer suggestion sent successfully.\nPlease wait for admin approval.");
                
            }
            else {
                ShowAlert(AlertTitle, @"Your beer suggestion is not sent.\nPlease try again.");
            }
        }
        else {
        }
