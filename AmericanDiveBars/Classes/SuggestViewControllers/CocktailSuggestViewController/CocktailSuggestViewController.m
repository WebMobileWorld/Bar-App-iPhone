//
//  CocktailSuggestViewController.m
//  AmericanDiveBars
//
...
static NSString * const kIngredientsPlaceholderText = @"Ingredients";
static NSString * const kHowToMakeItPlaceholderText = @"How to make it";


@interface CocktailSuggestViewController ()<MBProgressHUDDelegate,UzysAssetsPickerControllerDelegate,FPPopoverControllerDelegate>
{
    MBProgressHUD *HUD;
    ...
    BOOL showingIngredientsPlaceholder;
    BOOL showingHowToMakeItPlaceholder;
    
    NSString *cocktailName;
    NSString *ingredients;
    NSString *howToMakeIt;
    NSString *baseSpirit;
    NSString *type;
    NSString *served;
...
@implementation CocktailSuggestViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.titleView = [CommonUtils setTitleLabel:@"COCKTAIL SUGGEST"];
...
-(void)btnFilterClicked:(id)sender
{
    ...
    controller.suggestCocktailDelegate = self;
...
{
    return [self CocktailSuggestCellAtIndexPath:indexPath];
}

#pragma mark
#pragma mark CocktailSuggestCell Cell Configuration
- (CocktailSuggestCell *)CocktailSuggestCellAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString * const BeerSuggestCellIdentifier = @"CocktailSuggestCell";
    
    CocktailSuggestCell *cell = (CocktailSuggestCell *)[self.tblView dequeueReusableCellWithIdentifier:BeerSuggestCellIdentifier];
    if (cell == nil)
    {
        NSArray *nib;
        if([CommonUtils isiPad])
        {
            nib = [[NSBundle mainBundle] loadNibNamed:@"CocktailSuggestCell-ipad" owner:self options:nil];
        }
        else
        {
            nib = [[NSBundle mainBundle] loadNibNamed:@"CocktailSuggestCell" owner:self options:nil];
        }
        cell = [nib objectAtIndex:0];
    }
    
    cell.txtCocktailName.text = cocktailName;
    cell.txtIngredients.text = ingredients;
    cell.txtHowToMakeIt.text = howToMakeIt;
    cell.txtBaseSpirit.text = baseSpirit;
    cell.txtType.text = type;
    cell.txtServed.text = served;
    cell.txtPreparation.text = preparation;
    cell.txtStrength.text = strength;
    cell.txtDifficulty.text = difficulty;
    cell.txtFlavorProfile.text = flavorProfile;
    
    if (didSelectImage) {
        cell.imgUser.image = selectedImage;
    }
    else {
        cell.imgUser.image = [UIImage imageNamed:@"no_cocktail_detail.png"];
    }
...
-(void)btnSave_Clicked:(UIButton *)btnSave {
    ...
    CocktailSuggestCell *cell = (CocktailSuggestCell *)[self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:1]];
    
    if ([cocktailName length]==0) {
        cell.txtCocktailName.layer.borderColor = [UIColor redColor].CGColor;
        cell.txtCocktailName.layer.borderWidth = 0.0f;
        ShowAlert(AlertTitle, @"Please provide the Cocktail name.");
    }
    else {
        cell.txtCocktailName.layer.borderColor = [UIColor clearColor].CGColor;
        cell.txtCocktailName.layer.borderWidth = 0.0f;
        [self callCocktailSuggestWebservice];
    }
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if (textField.tag == 101) {
        cocktailName = textField.text;
    }
    
    else if (textField.tag == 102) {
        baseSpirit = textField.text;
    }
    ...
    if (textField.tag == 104) {
        served = textField.text;
    }
    
    else if (textField.tag == 105) {
        preparation = textField.text;
    }
    ...
    if (textField.tag == 107) {
        difficulty = textField.text;
    }
    
    else if (textField.tag == 108) {
        flavorProfile = textField.text;
    }
...
-(void)configureTextView:(CocktailSuggestCell *)cell withEditable:(BOOL)isEditable
{
    
    showingIngredientsPlaceholder = YES;
    if ([ingredients length]==0)
    {
        [cell.txtIngredients setText:kIngredientsPlaceholderText];
        [cell.txtIngredients setTextColor:[UIColor colorWithRed:146.0/255.0 green:145.0/255.0 blue:145.0/255.0 alpha:1.0]];
    }
    else
    {
        [cell.txtIngredients setText:ingredients];
        if (isEditable) {
            [cell.txtIngredients setTextColor:[UIColor whiteColor]];
        }
        else {
            [cell.txtIngredients setTextColor:[UIColor whiteColor]];
        }
        showingIngredientsPlaceholder = NO;
    }
    
    showingHowToMakeItPlaceholder = YES;
    if ([howToMakeIt length]==0)
    {
        [cell.txtHowToMakeIt setText:kHowToMakeItPlaceholderText];
        [cell.txtHowToMakeIt setTextColor:[UIColor colorWithRed:146.0/255.0 green:145.0/255.0 blue:145.0/255.0 alpha:1.0]];
    }
    else
    {
        [cell.txtHowToMakeIt setText:howToMakeIt];
        if (isEditable) {
            [cell.txtHowToMakeIt setTextColor:[UIColor whiteColor]];
        }
        else {
            [cell.txtHowToMakeIt setTextColor:[UIColor whiteColor]];
        }
        showingHowToMakeItPlaceholder = NO;
    }
...
-(void)textViewDidBeginEditing:(UITextView *)textView {
    ...
    if (textView.tag == 201) {
        ...
        if(showingIngredientsPlaceholder) {
            [textView setText:@""];
        ...
        // Check if it's showing a placeholder, remove it if so
        if(showingHowToMakeItPlaceholder) {
            [textView setText:@""];
            [textView setTextColor:[UIColor colorWithRed:146.0/255.0 green:145.0/255.0 blue:145.0/255.0 alpha:1.0]];
...
-(void)textViewDidEndEditing:(UITextView *)textView {
    if (textView.tag == 201) {
        ...
        if([[textView text] length] == 0 && !showingIngredientsPlaceholder) {
            [textView setText:kIngredientsPlaceholderText];
            [textView setTextColor:[UIColor colorWithRed:146.0/255.0 green:145.0/255.0 blue:145.0/255.0 alpha:1.0]];
        ...
        // Check the length and if it should add a placeholder
        if([[textView text] length] == 0 && !showingHowToMakeItPlaceholder) {
            [textView setText:kHowToMakeItPlaceholderText];
            [textView setTextColor:[UIColor colorWithRed:146.0/255.0 green:145.0/255.0 blue:145.0/255.0 alpha:1.0]];
...
-(void)callCocktailSuggestWebservice {
    [CommonUtils callWebservice:@selector(cocktailsuggest) forTarget:self];
}

#pragma mark - Web service Request

-(void)cocktailsuggest {
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@cocktailsuggest",WEBSERVICE_URL];
    ...
    for (int i=0; i<[self.aryImages count]; i++)
    {
        ...
        if ([mediaType isEqualToString:@"image"])
        {
            ...
            [body appendData:[[NSString stringWithFormat:@"Content-Disposition: multipart/form-data; name=\"beer_image\"; filename=\"image_%@\"\r\n",[[self.aryImages objectAtIndex:i] valueForKey:@"name"]] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    //cocktail_name
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"cocktail_name"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",cocktailName] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    //how_to_make_it
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"how_to_make_it"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",howToMakeIt] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //base_spirit
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"base_spirit"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",baseSpirit] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    //served
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"served"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",served] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //preparation
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"preparation"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",preparation] dataUsingEncoding:NSUTF8StringEncoding]];
    
    //strength
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"strength"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",strength] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    //flavor_profile
    [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"%@\"\r\n\r\n", @"flavor_profile"] dataUsingEncoding:NSUTF8StringEncoding]];
    [body appendData:[[NSString stringWithFormat:@"%@\r\n",flavorProfile] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        if (connectionError == nil) {
            ...
            if ([status isEqualToString:@"success"]) {
                ...
                [self.navigationController popViewControllerAnimated:YES];
                ShowAlert(AlertTitle, @"Your cocktail suggestion sent successfully.\nPlease wait for admin approval.");
                
            }
            else {
                ShowAlert(AlertTitle, @"Your cocktail suggestion is not sent.\nPlease try again.");
            }
        }
        else {
        }
