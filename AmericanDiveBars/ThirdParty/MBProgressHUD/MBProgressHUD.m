...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[MBRoundProgressView1 class]];
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[MBRoundProgressView1 alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(MBRoundProgressView1 *)indicator setAnnular:YES];
		}
	} 
...
@implementation MBRoundProgressView1
